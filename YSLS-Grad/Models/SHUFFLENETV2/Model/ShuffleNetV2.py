...
class GradientDescentUnivariateLassoRegression:
    ...
    def generate_synthetic_data(self):
        ...
        n_samples = 14400  # Adjust as needed
...
model = ShuffleNetV2(targets_size)
model.to(device)
...
# torch.save(model.state_dict(), 'ShuffleNetV2\ShuffleNetV2_File_4.pt')

model_filename = os.path.join(results_folder, 'ShuffleNetV2_File_4.pt')
...
# model.load_state_dict(torch.load('ShuffleNetV2\ShuffleNetV2_File_4.pt'))
# model.eval()
...
print("\n\nAccuracy Results of ShuffleNetV2:")
print("----------------------------------------------------------------")
...
# Gọi hàm plot_soft_threshold_and_segmentation để vẽ biểu đồ đường
...
results_txt_file2 = os.path.join(results_folder, "ShuffleNetV2_File_2.txt")
...
results_txt_file3 = os.path.join(results_folder, "ShuffleNetV2_File_3.txt")
...
results_normalized_confusion_matrix = os.path.join(
    results_folder, "Normalized_Confusion_Matrix.png")
...
df_normalized_cm = pd.DataFrame(
    normalized_cm, index=class_names, columns=class_names)
...
with open(results_txt_file3, "w", encoding="utf-8") as file:
    ...
    file.write("\nTraining Accuracies:\n")
    formatted_train_accuracies = [
        f"{accuracy:.5f}" for accuracy in train_accuracies]
    file.write(", ".join(formatted_train_accuracies) + "\n")

    # Ghi test accuracies
    file.write("\nTest Accuracies:\n")
    formatted_test_accuracies = [
        f"{accuracy:.5f}" for accuracy in test_accuracies]
